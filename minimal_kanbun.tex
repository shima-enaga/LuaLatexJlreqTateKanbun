...
\newfontfamily\tradfont[RawFeature={+trad}]{NotoSerifJP}
%
...
\usepackage[unit=1.5em, scale=3, kumi=aki, tateaki=2.5, fontcmd=\Asebi,
...
% 分で書くわけではない。上の\Kanbunから\EndKanbunの範囲を作成し、一度、コンパイ
% ルすると、このTeXファイルがあるディレクトリ（フォルダのこと）と同じところに
% kanbun_output.txtが作成される。そして、その内容を\begin{spacing}{1}から
% \end{spacing}で挟まれた部分にコピペして作る。
% しかる後に傍線を引きたいところに、\bousenコマンド、
% 行頭（縦書きの場合、正確には列の先頭）にレ点が来た場合には、\buntoureコマンド
% （ふりかな＋おくりかな）の長さが特に長い場合には適宜調整、
