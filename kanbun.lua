...
-- 改行コードあり・なしの文字列を格納するテーブル
local annotated_lines_table_with_newline = {}
local annotated_lines_table_without_newline = {}
...
function main_loop()
    ...
    for i,l in ipairs(verb_table) do
        ...
        -- some unknown bug (in \containskana ?) solved by changing the last entry to an empty string if not ascii
    ...
    for i,l in ipairs(lines_chars_table) do
        ...
        -- reverse the makeshift bug fix to \containskana
        line = string.gsub(line, "“⌊⌋”", "")
        ...
        for j,u in ipairs(units) do
            ...
            if u:match(tmp_number_of_multikanji_braces[1]) then
                ...
                units[j] = "\n\\kanjiunit{\\multifuriokuri["..multiruby_raise_by_.."]{"..right_furi_.."}{"..right_okuri_.."}}{"..left_punct_.."}{"..kanji_.."}{"..punct_.."}{"..kaeriten_.."}{\\multifuriokuri["..multiruby_raise_by_.."]{"..left_furi_.."}{"..left_okuri_.."}}}"
            else
                if kanji_ == "" then
                    units[j] = ""
                else
                    units[j] = "\n\\kanjiunit{\\furiokuri{"..right_furi_.."}{"..right_okuri_.."}}{"..left_punct_.."}{"..kanji_.."}{"..punct_.."}{"..kaeriten_.."}{\\furiokuri{"..left_furi_.."}{"..left_okuri_.."}}"
                end
            end
        end

        local line_with_newline = table.concat(units, "")
        local line_without_newline = remove_newlines(line_with_newline)

        table.insert(annotated_lines_table_with_newline, line_with_newline)
        table.insert(annotated_lines_table_without_newline, line_without_newline)
    end

    -- ouput

    local output_with_newline = "{\\kanbunfont" .. table.concat(annotated_lines_table_with_newline, "\\par") .. "\\par}"
    local output_without_newline = "{\\kanbunfont" .. table.concat(annotated_lines_table_without_newline, "\\par") .. "\\par}"

    for _, line in ipairs(annotated_lines_table_with_newline) do
        print(line)
    end

    local filename = "kanbun_output.txt"
    ...
    if file then
        ...
        file:close()
        print("ファイルに書き込みました: " .. filename)
    else
        print("ファイルを開けませんでした: " .. filename)
    end

--  print(output_without_newline)

    directtex("\\def\\printkanbun{" .. output_without_newline .. "}")
